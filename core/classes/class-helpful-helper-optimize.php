<?php
/**
 * Helper for optimize and maintain plugin
 *
 * @package Helpful
...
class Helpful_Helper_Optimize
{
    ...
    {        
        ...
        $response = [];
        $response = array_merge($response, Helpful_Helper_Values::tableExists('helpful'));
        $response = array_merge($response, Helpful_Helper_Values::tableExists('helpful_feedback'));
        $response = array_merge($response, self::_optimizeTables());
        $response = array_merge($response, self::_moveFeedback());
        $response = array_merge($response, self::_removeIncorrectEntries());
     ...
     * @return array responses
     */
    private static function _optimizeTables()
    {
        global $wpdb;
     ...
     */
    private static function _moveFeedback()
    {
        global $wpdb;
     ...
     * @return array responses
     */
    private static function _removeIncorrectEntries() 
    {
        global $wpdb;
        ...
        if ($items) {
            ...
            $response []= sprintf( 
                esc_html_x(
                    "%d incorrect entries have been removed from table '%s'", 
